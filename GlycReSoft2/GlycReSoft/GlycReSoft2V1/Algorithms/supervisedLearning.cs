...
namespace GlycReSoft
{
    public class supervisedLearning
    {
        ...
        public List<groupingResults>[] run(OpenFileDialog FileLinks, List<composition.comphypo> comhyp)
        {
            ...
            foreach (String filename in FileLinks.FileNames)
            {
                ...
                if (LRLR.Count == 1)
                {
                    ...
                    List<groupingResults> FinalResult = LRLR;
        ...
        public List<groupingResults>[] evaluate(OpenFileDialog FileLinks, List<composition.comphypo> comhyp, Feature dfeatureData)
        {
            //Initialize storage variables.
            List<groupingResults>[] AllFinalResults = new List<groupingResults>[Convert.ToInt32(FileLinks.FileNames.Count())];
            Int32 Count = 0;
        ...
        public Feature obtainFeatures(OpenFileDialog FileLinks, List<composition.comphypo> comhyp)
        {
        ...
        private List<groupingResults> Groupings(String filename, parameters.para paradata, Double Mas, List<composition.comphypo> comhyp)
        {
            ...
            List<groupingResults> sgResults = new List<groupingResults>();
                ...
                if (fgResults[j].mostAbundant == true)
                {
                    ...
                    for (int i = lastMatch; i < comhyp.Count; i++)
                    {
                        ...
                        if (fgResults[j].DeconRow.monoisotopic_mw > (comhyp[i].MW + MatchingError))
                        {
                            ...
                            {
                                groupingResults GR = new groupingResults();
                                composition.comphypo comhypi = new composition.comphypo();
                                GR = fgResults[j];
                                GR.Match = false;
                                GR.comphypo = comhypi;
                                sgResults.Add(GR);
        ...
        private groupingResults matchPassbyValue(groupingResults input1, composition.comphypo comhypo)
        {
            groupingResults storage = new groupingResults();
        ...
        private Feature logisRegression(List<groupingResults> LRLR)
        {
            int numofMatches = 0;
        ...
        private List<groupingResults> balanceMatch(List<groupingResults> LRLR)
        {
            List<groupingResults> MatchList = new List<groupingResults>();
            List<groupingResults> noMatchList = new List<groupingResults>();
            for (int i = 0; i < LRLR.Count; i++)
            {
                if (LRLR[i].Match == true)
                {
        ...
        public List<groupingResults> Scorings(List<groupingResults> LRLR, Feature featureData, parameters.para paradata)
        {
        ...
        private Double adductMass(List<composition.comphypo> comhyp)
        {
            String adduct = "";
            String replacement = "";
            try
